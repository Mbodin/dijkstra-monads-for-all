...
(************************************************************************)
(** * Some properties of the operators on srelations                     *)
(************************************************************************)
(** * Initial version by Bruno Barras                                   *)
(************************************************************************)

From Mon Require Import Base sprop.SPropBase.
From Mon Require Import SRelation.SRelation_Definitions.
From Mon Require Import SRelation.SRelation_Operators.
...
Section Properties.
  ...
  (** *** Contributed by P. Castéran *)
    ...
    Lemma clos_trans_t1n_iff : forall x y,
        clos_trans R x y s<-> clos_trans_1n R x y.
    Proof.
      split.
    ...
    Lemma clos_trans_tn1_iff : forall x y,
        clos_trans R x y s<-> clos_trans_n1 R x y.
    Proof.
      split.
    ...
    Lemma clos_rt_rt1n_iff : forall x y,
      clos_refl_trans R x y s<-> clos_refl_trans_1n R x y.
    Proof.
      split.
      apply clos_rt_rt1n.
      apply clos_rt1n_rt.
    ...
    Lemma clos_rt_rtn1_iff : forall x y,
        clos_refl_trans R x y s<-> clos_refl_trans_n1 R x y.
    Proof.
      split.
      apply clos_rt_rtn1.
      apply clos_rtn1_rt.
    ...
    Proof.
      ...
      apply H0 with x; auto with sets.
    ...
    Lemma rt1n_ind_right : forall (P : A -> SProp) (z:A),
      P z ->
    ...
    Lemma clos_refl_trans_ind_right : forall (P : A -> SProp) (z:A),
      P z ->
    ...
    Proof.
      ...
      match goal with [ H : _ s\/ _ |- _] => destruct H end ;
      [constructor 1|constructor 3; constructor 1]; auto.
    Qed.
    ...
    Lemma clos_rst_rst1n_iff : forall x y,
      clos_refl_sym_trans R x y s<-> clos_refl_sym_trans_1n R x y.
    Proof.
      split.
    ...
    Lemma clos_rst_rstn1_iff : forall x y,
      clos_refl_sym_trans R x y s<-> clos_refl_sym_trans_n1 R x y.
    Proof.
      split.
  ...
  Lemma clos_trans_transp_permute : forall x y,
    transp _ (clos_trans R) x y s<-> clos_trans (transp _ R) x y.
  Proof.
    split; induction 1;
